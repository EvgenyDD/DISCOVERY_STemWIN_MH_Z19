...
unsigned char SPI_RW_Byte(u8 num)
{
    ...
    while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_RXNE) == RESET)
        ;
    uint8_t ret = SPI_I2S_ReceiveData(SPI1);
...
void ts_callback(xTimerHandle pxTimer)
{
    //BSP_Pointer_Update();
    ...
    if(!GPIO_ReadInputDataBit(XPT2046_IRQ_PORT, XPT2046_IRQ_PAD))
    {
        ...
        TS_State.x = orient == PORTRAIT ? x : LCD_PIXEL_HEIGHT - y;
        TS_State.y = orient == PORTRAIT ? y : x;

        TS_State.Pressed = 1;
        TS_State.Layer = 0;

        if((x < 240 /*LCD_GetXSize()*/) && (y < 400 /*LCD_GetYSize()*/))
