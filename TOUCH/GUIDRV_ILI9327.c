...
#define yphys ((U16)y)
	...
	// Pack RGB
	color =
			((reads[1] & 0xF800) | ((reads[1] & 0x00FC) << 3) | (reads[2] >> 11));
...
static inline void _DrawBitmap1BPPOpaque(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, const U8 GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
 ...
 * @param   pTrans  Palette
 */
static inline void _DrawBitmap1BPPTransparent(GUI_DEVICE *pDevice, int x0,
		int y0, int width, int height, int stride, const U8 GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
 ...
 */
static inline void _DrawBitmap1BPPXOR(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, const U8 GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
 ...
 * @param   pTrans  Palette
 */
static inline void _DrawBitmap1BPP(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, U8 const GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
	printf("###N->%d   \n\t",
	...
	case 0:
		_DrawBitmap1BPPOpaque(pDevice, x0, y0, width, height, stride, pData,
				diff, pTrans);
		break;
	case LCD_DRAWMODE_TRANS:
		_DrawBitmap1BPPTransparent(pDevice, x0, y0, width, height, stride,
				pData, diff, pTrans);
		break;
	case LCD_DRAWMODE_XOR:
		_DrawBitmap1BPPXOR(pDevice, x0, y0, width, height, stride, pData, diff,
				pTrans);
		break;
	}
}
...
static inline void _DrawBitmap2BPPOpaque(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, const U8 GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
...
static inline void _DrawBitmap2BPPTransparent(GUI_DEVICE *pDevice, int x0,
		int y0, int width, int height, int stride, const U8 GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
	LCD_PIXELINDEX pixels;
	const U8 GUI_UNI_PTR *pTempData;
...
static inline void _DrawBitmap2BPP(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, U8 const GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
	switch (GUI_pContext->DrawMode & LCD_DRAWMODE_TRANS) {
	case 0:
		_DrawBitmap2BPPOpaque(pDevice, x0, y0, width, height, stride, pData,
				diff, pTrans);
		break;
	case LCD_DRAWMODE_TRANS:
		_DrawBitmap2BPPTransparent(pDevice, x0, y0, width, height, stride,
				pData, diff, pTrans);
		break;
		break;
	}
...
static inline void _DrawBitmap4BPPOpaque(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, const U8 GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
	LCD_PIXELINDEX pixels;
	const U8 GUI_UNI_PTR *pTempData;
 ...
 * @param   pTrans  If pTrans is not NULL, it points to the palette and pData points is color index
 */
static inline void _DrawBitmap4BPPTransparent(GUI_DEVICE *pDevice, int x0,
		int y0, int width, int height, int stride, const U8 GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
	LCD_PIXELINDEX pixels;
	const U8 GUI_UNI_PTR *pTempData;
 ...
 * @param   pTrans  If pTrans is not NULL, it points to the palette and pData points is color index
 */
static inline void _DrawBitmap4BPP(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, U8 const GUI_UNI_PTR *pData,
		int diff, const LCD_PIXELINDEX *pTrans) {
	switch (GUI_pContext->DrawMode & LCD_DRAWMODE_TRANS) {
	case 0:
		_DrawBitmap4BPPOpaque(pDevice, x0, y0, width, height, stride, pData,
				diff, pTrans);
		break;
	case LCD_DRAWMODE_TRANS:
		_DrawBitmap4BPPTransparent(pDevice, x0, y0, width, height, stride,
				pData, diff, pTrans);
		break;
	}
}
 ...
 * @param   pTrans  If pTrans is not NULL, it points to the palette and pData points is color index
 */
static inline void _DrawBitmap8BPPOpaque(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, U8 const GUI_UNI_PTR *pData,
 ...
 * @param   pTrans  If pTrans is not NULL, it points to a buffer palette and pData points to color index
 */
static inline void _DrawBitmap8BPPTransparent(GUI_DEVICE *pDevice, int x0,
		int y0, int width, int height, int stride, U8 const GUI_UNI_PTR *pData,
		const LCD_PIXELINDEX * pTrans) {
	LCD_PIXELINDEX pixel;
	int x1 = x0 + width - 1;
	int y1 = y0 + height - 1;
 ...
 * @param   pTrans  If pTrans is not NULL, it points to the palette and pData points is color index
 */
static inline void _DrawBitmap8BPP(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, U8 const GUI_UNI_PTR *pData,
		const LCD_PIXELINDEX *pTrans) {
	switch (GUI_pContext->DrawMode & LCD_DRAWMODE_TRANS) {
	case 0:
		_DrawBitmap8BPPOpaque(pDevice, x0, y0, width, height, stride, pData,
				pTrans);
		break;
	case LCD_DRAWMODE_TRANS:
		_DrawBitmap8BPPTransparent(pDevice, x0, y0, width, height, stride,
				pData, pTrans);
		break;
	}
}
 ...
 * @param   pData   Bitmap data
 */
static inline void _DrawBitmap16BPP(GUI_DEVICE *pDevice, int x0, int y0,
		int width, int height, int stride, U16 const GUI_UNI_PTR *pData) {
	DRIVER_CONTEXT *pContext = (DRIVER_CONTEXT *) pDevice->u.pContext;
	int x1 = x0 + width - 1;
	int y1 = y0 + height - 1;
#if (LCD_MIRROR_X == 1) || (LCD_MIRROR_Y == 1) || (LCD_SWAP_XY == 1)
 ...
 * @param   pTrans  If pTrans is not NULL, it points to the palette and pData points is color index
 */
static void _DrawBitmap(GUI_DEVICE *pDevice, int x0, int y0, int width,
		int height, int bpp, int stride, const U8 GUI_UNI_PTR *pData, int diff,
		const LCD_PIXELINDEX *pTrans) {
	printf("_DrawBitmap->%d %dx%d - %dx%d - str%d - %c - diff%d \n\r", bpp, x0,
			y0, width, height, stride, (char) *pData, diff);

	switch (bpp) {
	case 1:
		_DrawBitmap1BPP(pDevice, x0, y0, width, height, stride,
				(const U8 *) pData, diff, pTrans);
		break;
	case 2:
		_DrawBitmap2BPP(pDevice, x0, y0, width, height, stride,
				(const U8 *) pData, diff, pTrans);
		break;
	case 4:
		_DrawBitmap4BPP(pDevice, x0, y0, width, height, stride,
				(const U8 *) pData, diff, pTrans);
		break;
	case 8:
		_DrawBitmap8BPP(pDevice, x0, y0, width, height, stride,
				(const U8 *) pData, pTrans);
		break;
	case 16:
		_DrawBitmap16BPP(pDevice, x0, y0, width, height, stride,
				(const U16 *) pData);
		break;
	}
}
...
static void _ReadRect(GUI_DEVICE *pDevice, int x0, int y0, int x1, int y1,
...
#define y1phys ((U16)y1)
	...
	while (total > 1) {
		...
		*pBuf = ((read[0] & 0xF800) | ((read[0] & 0x00FC) << 3)
				| (read[1] >> 11));
		++pBuf;
		*pBuf = (((read[1] & 0x00F8) << 8) | ((read[2] & 0xFC00) >> 5)
				| ((read[2] & 0x00F8) >> 3));
		++pBuf;
		total -= 2;
	}
...
static void (* _GetDevFunc(GUI_DEVICE **ppDevice, int index))(void)
		...
		const GUI_DEVICE_API GUIDRV_ILI9327_API = {
				...
				_SetPixelIndex, _XorPixel,
				// Set origin
				_SetOrg,
				// Request information
				_GetDevFunc, _GetDevProp, _GetDevData, _GetRect, };
